...
void Hormiga::SetTarget(Comida* h) {
	p_target = h;
}

void Hormiga::SetTargetM(Matador* h) {
	p_targetM = h;
}
...
Matador* Hormiga::GetTargetM() {
	return p_targetM;
}

void Hormiga::ChaseScape() {
	Vector2f posActual = m_spr.getPosition();
	Vector2f posKiller = p_targetM->GetPosition();
	dir = posKiller - posActual;
	float norm = -sqrt(dir.x * dir.x + dir.y * dir.y);
	dir.x /= norm;
	dir.y /= norm;
}